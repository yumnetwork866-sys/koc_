...
const SellerAffiliatePanel = () => {
  ...
  const [performanceTableTab, setPerformanceTableTab] = useState('creator');
  ...
  const performanceBreakdown = section === 'performance' && performanceTableTab === 'creator'
  ...
  const selectedPerformanceExport = performanceTableTab === 'base'
    ? data.base_snapshot_export
    : data.snapshot_export;
  ...
  const changeSection = (value) => { setSection(value); setStatus(value === 'target' ? 'ONGOING' : ''); setPerformanceTableTab('creator'); setPageTokens([]); setData({}); setError(''); };
  const submitSearch = (event) => { event.preventDefault(); setPageTokens([]); setSubmittedKeyword(keyword.trim()); };
  const syncPerformance = async () => {
    if (!shopId || performanceSyncing) return;
    setPerformanceSyncing(true);
    setError('');
    try {
      const result = await syncTikTokCreatorPerformance(shopId, { windowType: performanceWindow, planType: 'ALL' });
      setData((current) => ({ ...current, export: result.export, base_export: result.base_export }));
    } catch (err) {
    ...
    <div className="page seller-affiliate">
      ...
      <section className="section-card seller-affiliate__controls">
        <div className="seller-affiliate__filter-grid">
          ...
          {section === 'performance' ? <button className="button seller-affiliate__generate-report" type="button" onClick={syncPerformance} disabled={performanceSyncing}>{performanceSyncing ? t('sellerAffiliate.performanceStatus_PROCESSING') : t('sellerAffiliate.generateReport')}</button> : null}
        </div>
      </section>
