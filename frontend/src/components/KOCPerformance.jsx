        ...
        <div id="performance-panel" role="tabpanel" aria-labelledby="performance-tab" className="koc-tab-panel">
          ...
          <section className="koc-chart-grid">
            <article className="section-card koc-chart-card" aria-labelledby="top-koc-chart-title">
              ...
              {topChartData.length ? <div className="koc-chart" role="img" aria-label={`${t('koc.top10Chart')} · ${t(chartMetric === 'engagement' ? 'koc.engagementRate' : chartMetric === 'videoCount' ? 'koc.videos' : 'koc.totalViews')}`}><ResponsiveContainer width="100%" height="100%"><BarChart data={topChartData} margin={{ top: 8, right: 8, bottom: 46, left: 4 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="name" angle={-32} textAnchor="end" interval="preserveStartEnd" minTickGap={18} height={70} /><YAxis width={56} tickFormatter={(value) => chartMetric === 'engagement' ? `${value}%` : Intl.NumberFormat(locale, { notation: 'compact' }).format(value)} /><Tooltip formatter={(value) => chartMetric === 'engagement' ? formatPercent(value) : formatNumber(value)} /><Bar dataKey="value" fill="var(--color-accent)" radius={[6, 6, 0, 0]} /></BarChart></ResponsiveContainer></div> : <div className="empty-state">{t('koc.noData')}</div>}
            </article>
            <article className="section-card koc-chart-card" aria-labelledby="weekly-chart-title">
              ...
              {kpis?.weeklyViews?.length ? <div className="koc-chart" role="img" aria-label={t('koc.weeklyViews')}><ResponsiveContainer width="100%" height="100%"><LineChart data={kpis.weeklyViews} margin={{ top: 8, right: 16, bottom: 12, left: 4 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="week" tickFormatter={(value) => new Intl.DateTimeFormat(locale, { month: 'short', day: 'numeric' }).format(new Date(value))} /><YAxis width={56} tickFormatter={(value) => Intl.NumberFormat(locale, { notation: 'compact' }).format(value)} /><Tooltip labelFormatter={(value) => formatDate(value)} formatter={(value) => formatNumber(value)} /><Line type="monotone" dataKey="views" stroke="var(--color-accent)" strokeWidth={3} dot={{ r: 3 }} activeDot={{ r: 5 }} /></LineChart></ResponsiveContainer></div> : <div className="empty-state">{t('koc.noWeeklyViews')}</div>}
            </article>
          </section>
